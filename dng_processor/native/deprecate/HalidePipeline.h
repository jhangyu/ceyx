...
public:
  ...
  /// Phase 10: YCbCr (Lossy DNG) -> RGB processing
  /// @param yPlane            Y plane buffer (full resolution, width×height)
  /// @param uPlane            U plane buffer (YUV420: width/2 × height/2)
  /// @param vPlane            V plane buffer (YUV420: width/2 × height/2)
  /// @param width             Image width
  /// @param height            Image height
  /// @param camToSrgb         Camera->sRGB [9] row-major composite matrix
  /// @param baselineExposure  BaselineExposure in EV
  ...
  /// @return Heap-allocated RGBA buffer (caller frees with delete[]).
  ///         Returns nullptr on failure.
  static uint8_t *processYCbCr(const uint8_t *yPlane, const uint8_t *uPlane,
