...
")
        # W-CI1: ZLIB shim, Windows only. Android does NOT need one -- the
        # NDK sysroot ships libz.so + zlib.h, so RawSpeed3's real
        # find_package(ZLIB) succeeds there (third_party.cmake:10-23 probes the
        # same sysroot copy). Windows has no system zlib at all:
        # third_party.cmake:33-59 FetchContent's zlib 1.3.1 and aliases
        # ZLIB::ZLIB -> zlibstatic, but an ALIAS target does not make
        # module-mode find_package(ZLIB) succeed, so RawSpeed3
        # (cmake/src-dependencies.cmake:190-196) hard-errors.
        #
        # Deliberately NOT using RawSpeed3's own -DUSE_BUNDLED_ZLIB=ON escape:
        # that makes RawSpeed3 build a SECOND zlib in the same build tree,
        # colliding with the zlibstatic target third_party.cmake already
        # created, and would link two zlib copies into one image.
        #
        # Version note: RawSpeed3 asks for ZLIB 1.2.11. This module reports
        # FOUND unconditionally, so the request is satisfied by assertion
        # rather than by comparison -- honest here only because the vendored
        # copy is pinned to 1.3.1 (third_party.cmake:44), which really does
        # exceed the floor. Re-check this if that pin ever moves backwards.
        if(WIN32)
            file(WRITE ${_dng_jpeg_shim_dir}/FindZLIB.cmake
"# Generated by cmake/tests.cmake for Windows; resolves ZLIB to the vendored
# zlib built by cmake/third_party.cmake in this build tree.
set(ZLIB_FOUND TRUE)
set(ZLIB_INCLUDE_DIRS \"${dng_zlib_SOURCE_DIR};${dng_zlib_BINARY_DIR}\")
set(ZLIB_INCLUDE_DIR \"${dng_zlib_SOURCE_DIR}\")
