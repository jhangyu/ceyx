#!/usr/bin/env bash
# Task 6 / gate G-B -- deploy to a real Android device, run the gate THREE
...
BIN="${HERE}/build/test_orient_vk_device"
DEV_DIR=/data/local/tmp/ceyx_orient_gate
ART="${REPO}/tmp/verify/orient_prod_t6_device_gate.txt"
SYMS="${REPO}/tmp/verify/orient_prod_t6_deployed_syms.txt"
...

{
  ...
  shasum -a 256 "${HERE}/build/aot/k_vk_gt_on.a" "${HERE}/build/aot/k_vk_gt_off.a"
  ls -l "${HERE}/build/aot/k_vk_gt_on.a" "${HERE}/build/aot/k_vk_gt_off.a" | awk '{print $5, $NF}'
} >> "${ART}"

adb shell "rm -rf ${DEV_DIR}; mkdir -p ${DEV_DIR}"
adb push "${BIN}" "${DEV_DIR}/test_orient_vk_device" >/dev/null
adb shell "chmod 755 ${DEV_DIR}/test_orient_vk_device"
...
adb shell "md5sum ${DEV_DIR}/test_orient_vk_device" | tee -a "${ART}"
adb pull "${DEV_DIR}/test_orient_vk_device" "${HERE}/build/pulled_from_device" >/dev/null
"${NM}" "${HERE}/build/pulled_from_device" > "${SYMS}" 2>&1 || true
{
  echo "=== DEPLOYED SYMBOL PROOF (nm-to-file, then grep) ==="
  echo "SYMS_FILE=${SYMS}"
  echo "PULLED_SHA256=$(shasum -a 256 "${HERE}/build/pulled_from_device" | awk '{print $1}')"
...
for i in 1 2 3; do
  {
    echo "=== RUN ${i}/3 ==="
    adb shell "cd ${DEV_DIR} && LD_LIBRARY_PATH=${DEV_DIR} ./test_orient_vk_device 2>&1; echo DEVICE_RC=\$?"
  } >> "${ART}" 2>&1
done
