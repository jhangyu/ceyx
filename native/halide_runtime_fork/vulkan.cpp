// =============================================================================
// [DNG-FORK R3-3] Forked copy of Halide v21.0.0 src/runtime/vulkan.cpp
// Upstream: https://github.com/halide/Halide/blob/v21.0.0/src/runtime/vulkan.cpp
// Pristine copy: ./upstream/vulkan.cpp (diff against it for the delta).
//
// Purpose: VkPipelineCache cross-launch persistence. This TU is compiled into
// the Android build (CMake option DNG_VK_PIPELINE_CACHE) and overrides the
// entire Vulkan runtime module of halide_runtime.a via weak-symbol precedence
// (all halide_vulkan_* entry points are weak in the generated runtime, and
// object files precede archives on the link line). Module globals
// (compilation_cache, cached_device, vk function pointers, ...) are weak
// EXTERNAL symbols, so the fork and the archive share one winning copy of all
// state — the archive's vulkan code simply becomes dead.
//
// Changes vs upstream (all tagged [DNG-FORK R3-3]):
//   1. vulkan_resources.h is the patched sibling copy (pipeline-cache usage).
//   2. halide_vulkan_device_release: flush + destroy the persistent cache
//      before the VkDevice is destroyed.
//   3. register_vulkan_allocation_pool constructor REMOVED in this copy: the
//      archive's own (anonymous-namespace) constructor still runs from its
//      .init_array and registers the shared weak-external pool; keeping a
//      second constructor here would register the same pool twice and
...
WEAK int halide_vulkan_device_release(void *user_context) {
    ...
    if (acquire_status == halide_error_code_success) {
        ...
        if ((instance == cached_instance) && (device == cached_device)) {
            // [DNG-FORK R3-3] Flush the persistent pipeline cache to disk and
            // destroy the VkPipelineCache object before its device dies.
            dng_vkpc_on_device_release(user_context, allocator);
...
// [DNG-FORK R3-3] FFI-facing pipeline-cache controls. New strong symbols (not
// present upstream); dng_ffi_api.cpp references them as weak imports so
// builds without this fork TU (macOS, or DNG_VK_PIPELINE_CACHE=OFF) still link.

// Set (or clear, with nullptr/"") the cache file path. Should be called
// before the first decode/warmup; a later change only affects future saves.
// Returns 0 always (never fails a caller).
int dng_vk_pipeline_cache_set_path(const char *path) {
    ...
    memcpy(dng_vkpc_path, path, n);
    dng_vkpc_path[n] = '\0';
    return 0;
}

// Serialize the cache to disk if dirty. 0 = saved or nothing to do, -1 = I/O
// or driver failure (informational only; never affects decoding).
int dng_vk_pipeline_cache_save(void) {
    return dng_vkpc_save(nullptr);
}

// Observability bitmask for cache-hit double evidence:
//   bit0 (1) = feature enabled (path resolved, not killed by env)
//   bit1 (2) = VkPipelineCache object exists this session
//   bit2 (4) = cache file was loaded & accepted at creation (cross-launch HIT)
//   bit3 (8) = unsaved pipeline data pending (dirty)
int dng_vk_pipeline_cache_status(void) {
    ScopedSpinLock lock(&dng_vkpc_lock);
    int status = 0;
    if (dng_vkpc_effective_path() != nullptr) {
        status |= 1;
    }
...
// Payload bytes accepted from the cache file at creation (0 = cold start).
int64_t dng_vk_pipeline_cache_loaded_bytes(void) {
    ScopedSpinLock lock(&dng_vkpc_lock);
    return (int64_t)dng_vkpc_loaded_bytes;
}

// [DNG-FORK R3-3 Option B] Eagerly create the Vulkan context (if needed) and
// the persistent VkPipelineCache object, so callers can query a meaningful
// cross-launch HIT status (bit 4) BEFORE any dispatch — the cache object is
// otherwise created lazily at first pipeline creation. Used by the warmup
// path to decide whether full-size prewarm dispatches can be skipped.
// Never fails the caller: on any error it simply returns the current status
// (bit 4 unset => callers run the full prewarm, i.e. upstream behavior).
int dng_vk_pipeline_cache_prepare(void) {
    ...
    uint32_t queue_family_index = 0;
    VkDebugUtilsMessengerEXT messenger = VK_NULL_HANDLE;
    int error_code = halide_vulkan_acquire_context(
        nullptr, &allocator, &instance, &device, &physical_device,
        &queue, &queue_family_index, &messenger, true);
    if (error_code == halide_error_code_success) {
        if (allocator != nullptr) {
            (void)dng_vkpc_acquire(nullptr, reinterpret_cast<VulkanMemoryAllocator *>(allocator));
        }
        halide_vulkan_release_context(nullptr, instance, device, queue, messenger);
    }
    return dng_vk_pipeline_cache_status();
}

namespace {

// [DNG-FORK R3-3] Upstream defines an anonymous-namespace
// __attribute__((constructor)) register_vulkan_allocation_pool() here. It is
// intentionally OMITTED in this fork copy: the halide_runtime.a archive's own
// local constructor still runs from its .init_array and registers the shared
// weak-external vulkan_allocation_pool (whose release_unused pointer resolves
// to this TU's halide_vulkan_release_unused_device_allocations override). A
// second registration would set pool->next = pool and hang every pool walk.
