...
// =============================================================================
// [DNG-FORK R3-3] Forked copy of Halide v21.0.0 src/runtime/vulkan_resources.h
// Upstream: https://github.com/halide/Halide/blob/v21.0.0/src/runtime/vulkan_resources.h
// Pristine copy: ./upstream/vulkan_resources.h (diff against it for the delta).
// Changes (all tagged [DNG-FORK R3-3]):
//   1. Include dng_vk_pipeline_cache_persist.h (persistence logic lives there).
//   2. vk_create_compute_pipeline: use the persistent VkPipelineCache instead
//      of VK_NULL_HANDLE; on failure with a cache, retry cacheless (cache I/O
//      or driver rejection must never fail a decode); mark cache dirty on
//      successful creation.
// =============================================================================
...
// [DNG-FORK R3-3] persistence logic (must follow vulkan_memory.h).
#include "dng_vk_pipeline_cache_persist.h"
...
int vk_create_compute_pipeline(void *user_context,
    ...
    // [DNG-FORK R3-3] Use the persistent pipeline cache when enabled.
    // dng_vkpc_acquire returns VK_NULL_HANDLE whenever the feature is
    // disabled or unavailable, which reproduces upstream behavior exactly.
    VkPipelineCache dng_pipeline_cache = dng_vkpc_acquire(user_context, allocator);
    VkResult result = vkCreateComputePipelines(allocator->current_device(), dng_pipeline_cache, 1, &compute_pipeline_info, allocator->callbacks(), compute_pipeline);
    if ((result != VK_SUCCESS) && (dng_pipeline_cache != VK_NULL_HANDLE)) {
        // [DNG-FORK R3-3] Red line: the cache must never fail a decode —
        // retry exactly as upstream would have called it.
        result = vkCreateComputePipelines(allocator->current_device(), VK_NULL_HANDLE, 1, &compute_pipeline_info, allocator->callbacks(), compute_pipeline);
    }
    if (result != VK_SUCCESS) {
        error(user_context) << "Vulkan: Failed to create compute pipeline! vkCreateComputePipelines returned " << vk_get_error_name(result) << "\n";
        return halide_error_code_generic_error;
    }
    if (dng_pipeline_cache != VK_NULL_HANDLE) {
        // [DNG-FORK R3-3] New pipeline state may have been added to the cache.
        dng_vkpc_mark_dirty();
    }
