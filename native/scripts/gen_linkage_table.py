...
# Markdown table header, kept as a single multi-line string constant rather
# than individual elements of the `lines` list `render()` builds below:
# native/scripts/deps/test_no_shell_lint.py's shell-lint sweep flags any
# argv-shaped list element containing "|" (the heuristic that guards the
# nm|grep pipefail class), and these two header lines are markdown table
# syntax, not shell argv -- a false positive the lint must keep firing on
# real violations for, so the fix is restructuring this literal, not
# weakening or allowlisting the lint.
_TABLE_HEADER = """<!-- linkage_table.md -- GENERATED FILE, do not hand-edit. -->
