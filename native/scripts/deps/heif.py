...
def traits(platform: str) -> dict[str, Any]:
    if platform not in _PLATFORM_TRAITS:
        raise HeifError(
            f"[heif] the HEIF stack assembly covers macOS and Linux only; "
            f"{platform!r} is built by build_heif_dist_windows.sh (see "
            f"manifest.toml [component.libde265.source.windows])"
        )
...
def default_dist(native_dir: Path, arch: str) -> Path:
...
def stamp_is_current(dist: Path, loaded: dict[str, Any], arch: str, platform: str) -> bool:
    ...
    if not stamp.is_file():
        return False
    if stamp.read_text(encoding="utf-8") != want_pins(loaded, arch):
        return False
...
# --------------------------------------------------------------------------
def build_kvazaar(loaded: dict[str, Any], platform: str, arch: str, dist: Path, stage: Path) -> None:
...
# --------------------------------------------------------------------------
def build_libheif(loaded: dict[str, Any], platform: str, arch: str, dist: Path, stage: Path) -> None:
    t = traits(platform)
    dist = Path(dist)
    for dep in (dist / t["de265_lib"], dist / _KVAZAAR_LIB, dist / _AOM_LIB):
        if not dep.is_file():
            raise _fail(f"{dep} missing -- build libde265/kvazaar/aom first")
    execute_mod.build_component(loaded, "libheif", platform, arch, dist, stage)


# --------------------------------------------------------------------------
# Stage: assemble (assertions + licences + stamp + cleanup)
# --------------------------------------------------------------------------
def assemble(loaded: dict[str, Any], platform: str, arch: str, dist: Path, stage: Path) -> None:
    t = traits(platform)
    dist = Path(dist)
    stage = Path(stage)
    ...
    assert_libheif_capabilities(platform, heif_lib)
    assert_arch(platform, arch, [heif_lib, de265_lib])
    vendor_licences(loaded, dist, stage)

    (dist / ".pins").write_text(want_pins(loaded, arch), encoding="utf-8")
    cleanup_stage(loaded, stage)
    _log(f"dist ready at {dist} (arch {arch})")
    for name in ("libheif", "libde265", "kvazaar", "aom"):
        _log(f"  {name:<8} {_component(loaded, name)['version']}")


def read_symbols(platform: str, library: Path) -> str:
    """Capture the symbol table as text. See the module docstring on why this
    is a capture-then-match and never a pipeline."""
    t = traits(platform)
    return run(["nm", *t["nm_flags"], str(library)]).stdout


def read_dependencies(platform: str, library: Path) -> str:
    t = traits(platform)
    return run([*t["deps_argv"], str(library)]).stdout


def assert_libheif_capabilities(platform: str, heif_lib: Path) -> None:
    """Proof, not assumption. Every check below reads the ARTEFACT."""
    symbols = read_symbols(platform, heif_lib)
    dependencies = read_dependencies(platform, heif_lib)
    check_symbols(symbols, dependencies)


def check_symbols(symbols: str, dependencies: str) -> None:
...
def assert_arch(platform: str, arch: str, libraries: list[Path]) -> None:
...
def cleanup_stage(loaded: dict[str, Any], stage: Path) -> None:
    """Remove the build and source trees. No build-de265/build-aom entries:
    those stages build nothing since D5, so nothing of theirs is ever staged.
    Pre-D5 leftovers are removed by the stamp change forcing a fresh dist
    rather than by naming paths this script can no longer create."""
    stage = Path(stage)
    targets = [stage / "build-kvazaar", stage / "build-libheif"]
    for name in ("libheif", "libde265", "kvazaar"):
        targets.append(stage / f"{name}-{_component(loaded, name)['version']}")
...
def build(
    ...
    *,
    ...
    if stage_arg == "all" and stamp_is_current(dist, loaded, arch, platform):
    ...
    if stage_arg in ("all", "kvazaar"):
        build_kvazaar(loaded, platform, arch, dist, stage)
    if stage_arg in ("all", "aom"):
        build_aom(loaded, dist)
    if stage_arg in ("all", "libheif"):
        build_libheif(loaded, platform, arch, dist, stage)
    if stage_arg in ("all", "assemble"):
        assemble(loaded, platform, arch, dist, stage)
    return 0
