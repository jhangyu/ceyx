...
- Linkage: **statically linked** on every platform.
  - macOS/host: Homebrew's `libjpeg.a` (`brew --prefix jpeg-turbo`).
    Static linking is deliberate — see `dng_processor/native/CMakeLists.txt` (`elseif(APPLE)` branch);
  ...
  - Android: built from `dng_processor/native/third_party/libjpeg-turbo`.
- License: IJG (Independent JPEG Group) License + Modified 3-clause BSD License;
  the SIMD sources are zlib-licensed.
- Text: `dng_processor/native/third_party/libjpeg-turbo/LICENSE.md`
  and `dng_processor/native/third_party/libjpeg-turbo/README.ijg`.
...
- Text: `dng_processor/native/third_party/dng_sdk/LICENSE`.
...
- Linkage: statically linked runtime. The Halide binary distribution itself is not
  ...
  `dng_processor/native/scripts/fetch_halide_v21_dist.sh` into
